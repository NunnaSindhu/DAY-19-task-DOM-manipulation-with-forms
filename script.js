...
let checkboxInput = document.querySelectorAll(".ckeckbox");
...
//create a function for radio buttons
...
function onEdit(td) {
  ...
  let genderRadio = selectedRow.cells[2].innerHTML;
  for (var radio of radios) {
    if (genderRadio.includes(radio.value)) {
      radio.checked = true;
    }
  }
...
function validateForm() {
  ...
  for (let i = 0; i < checkboxInput.length; i++) {
    if (checkboxInput[i].checked) {
      checkboxSelectedCount += 1;
    }
  }
  if (checkboxSelectedCount < 2) {
